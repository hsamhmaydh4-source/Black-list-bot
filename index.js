...
function dashboard(){return bg()+'<div class="app">'+side()+'<div class="main"><header class="topbar"><div class="crumb">السيرفرات　‹　<span data-guild-name>NEXUS COMMUNITY</span>　‹　'+names[current]+'</div><button class="search">بحث سريع　⌘ K</button></header><main class="content"><div class="heading"><div><div class="eyebrow">'+icon[current]+' وحدة '+names[current]+'</div><h1>'+ (current==='overview'?'صباح الخير، Admin .':names[current])+'</h1><p class="muted">'+(current==='overview'?'هذه هي الصورة الكاملة لنشاط سيرفرك اليوم.':'تحكم بكل إعدادات هذه الوحدة من مساحة واضحة وسريعة.')+'</p></div><button class="cta" onclick="saveSetting()">+　حفظ الإعدادات</button></div>'+ (current==='overview'?overview():generic())+'</main></div></div>'}
function verifying(){return bg()+'<main class="picker"><div class="card"><div class="eyebrow">NEXUS SECURITY</div><h2>جارٍ التحقق من السيرفر</h2><p class="muted">نتحقق من صلاحيات Discord ومن وجود البوت قبل إظهار أدوات التحكم.</p><div class="loader"></div></div></main>'}
async function verifyAndRender(){const response=await fetch('/api/guilds/'+encodeURIComponent(selectedGuildId)+'/summary');if(response.status===401){location.href='/auth/discord';return}if(response.status===403){document.getElementById('view').innerHTML=bg()+'<main class="picker"><div class="card"><h2>لا تملك صلاحية إدارة هذا السيرفر</h2><p class="muted">اختر سيرفرًا تملكه أو لديك فيه Manage Server.</p><button class="primary" onclick="location.href=&quot;/servers&quot;">العودة للسيرفرات</button></div></main>';return}if(response.status===409){document.getElementById('view').innerHTML=bg()+'<main class="picker"><div class="card"><h2>البوت غير مثبت في هذا السيرفر</h2><p class="muted">أضف البوت إلى السيرفر المختار ثم أعد المحاولة.</p><a class="primary" href="https://discord.com/oauth2/authorize?client_id=${CLIENT_ID || ""}&scope=bot%20applications.commands&permissions=8">إضافة البوت</a></div></main>';return}if(!response.ok){location.href='/servers';return}guildVerified=true;guildVerified=false;document.getElementById('view').innerHTML=verifying();verifyAndRender()}
function overview(){return '<div class="live"><span class="dot"></span><div><strong data-stat="status">جاري التحقق من حالة البوت</strong><span data-stat="statusDetail">يتم تحميل بيانات السيرفر الحقيقي...</span></div><div class="spacer"></div><b><span data-stat="channels">—</span> قنوات　 <span data-stat="roles">—</span> رتب</b></div><div class="stats"><div class="stat"><div class="stat-icon">○</div><div><span>إجمالي الأعضاء</span><strong data-stat="members">—</strong><small>من Discord مباشرة</small></div></div><div class="stat"><div class="stat-icon">◒</div><div><span>الأعضاء المتصلون</span><strong data-stat="online">—</strong><small>حالة حقيقية</small></div></div><div class="stat"><div class="stat-icon">□</div><div><span>القنوات</span><strong data-stat="channelsCard">—</strong><small>في السيرفر</small></div></div><div class="stat"><div class="stat-icon">◇</div><div><span>حالة الحماية</span><strong data-stat="protection">—</strong><small>من إعدادات البوت</small></div></div></div><div class="layout"><section class="panel"><div class="barhead"><div><h3>بيانات السيرفر</h3><span class="sub">معلومات مباشرة من Discord والبوت</span></div></div><div class="activity"><div>اسم السيرفر<small data-stat="guildName">جارٍ التحميل...</small></div><div>معرف السيرفر<small data-stat="guildId">'+selectedGuildId+'</small></div><div>حالة البوت<small data-stat="botStatus">جارٍ التحقق...</small></div></div></section><section class="panel"><div class="barhead"><div><h3>حالة الحماية</h3><span class="sub">التحكم الفعلي بالأنظمة</span></div></div><div class="score"><div class="ring" data-stat="score">—</div><div><strong data-stat="protectionLabel">جارٍ التحميل</strong><div class="sub">المفاتيح التالية مرتبطة بهذا السيرفر</div></div></div><div class="rows"><div class="row">الحماية المتقدمة <button class="toggle" data-setting="antiNuke" onclick="toggleSetting(this)"><i></i></button></div><div class="row">منع الروابط <button class="toggle" data-setting="antiLinks" onclick="toggleSetting(this)"><i></i></button></div><div class="row">مكافحة السبام <button class="toggle" data-setting="antiSpam" onclick="toggleSetting(this)"><i></i></button></div></div></section></div><div class="layout"><section class="panel"><div class="barhead"><div><h3>السجلات</h3><span class="sub">الأحداث الحقيقية ستظهر بعد ضبط قناة Logs</span></div></div><div class="activity"><div>مصدر البيانات<small>Discord Gateway وذاكرة البوت الحالية</small></div><div>الإعدادات<small>محفوظة لهذا guildId فقط</small></div></div></section><section class="panel"><div class="barhead"><div><h3>صلاحيات الإدارة</h3><span class="sub">تم التحقق من Manage Server عبر OAuth2</span></div></div><div class="activity"><div>السيرفر المختار<small data-stat="permission">جارٍ التحقق...</small></div></div></section></div>'}
...
async function loadSummary(){if(!selectedGuildId)return;try{const response=await fetch('/api/guilds/'+selectedGuildId+'/summary');if(response.status===409){document.getElementById('view').innerHTML='<div class="card"><h2>البوت غير مثبت في هذا السيرفر</h2><p class="muted">أضف البوت إلى السيرفر المختار أولًا، ثم أعد فتح لوحة التحكم.</p><a class="primary" href="https://discord.com/oauth2/authorize?client_id='+encodeURIComponent(''+location.clientId||'')+'&scope=bot%20applications.commands&permissions=8">إضافة البوت إلى السيرفر</a></div>';return}if(!response.ok)return;const data=await response.json();document.querySelectorAll('[data-guild-name]').forEach(el=>el.textContent=data.guild.name);for(const [key,value] of Object.entries({members:data.guild.memberCount,online:data.guild.online,channels:data.guild.channels,channelsCard:data.guild.channels,roles:data.guild.roles,guildName:data.guild.name,guildId:data.guild.id,botStatus:data.bot.ready?'متصل ويعمل':'قيد التشغيل',permission:data.permission?'مسموح':'مرفوض',protection:data.protection,score:data.protection==='مفعلة'?'94':'—',protectionLabel:data.protection==='مفعلة'?'مفعلة':'غير مفعلة',status:data.bot.ready?'كل الأنظمة تعمل بشكل طبيعي':'البوت قيد الاتصال',statusDetail:data.bot.ready?'بيانات حقيقية من السيرفر المختار':'انتظر اكتمال اتصال البوت'})){document.querySelectorAll('[data-stat="'+key+'"]').forEach(el=>el.textContent=value)}}catch(e){console.warn('summary load failed',e)}}async function loadSettings(){if(!selectedGuildId)return;try{const response=await fetch('/api/settings/'+selectedGuildId);if(response.status===401){location.href='/auth/discord';return}const data=await response.json();document.querySelectorAll('[data-setting]').forEach(el=>{el.classList.toggle('off',data[el.dataset.setting]===false)})}catch(e){console.warn('settings load failed',e)}}async function toggleSetting(el){if(!guildVerified||!selectedGuildId)return;el.classList.toggle('off');const payload={};document.querySelectorAll('[data-setting]').forEach(item=>payload[item.dataset.setting]=!item.classList.contains('off'));await fetch('/api/settings/'+selectedGuildId,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).catch(()=>null)}async function saveSetting(){if(!guildVerified||!selectedGuildId)return;const payload={};document.querySelectorAll('[data-setting]').forEach(item=>payload[item.dataset.setting]=!item.classList.contains('off'));const response=await fetch('/api/settings/'+selectedGuildId,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).catch(()=>null);if(response&&response.ok)alert('تم حفظ إعدادات البوت بنجاح.')}function selectGuild(id){selectedGuildId=id;history.pushState({},'', '/dashboard/'+encodeURIComponent(id)+'/overview');render()}function go(section){history.pushState({},'', '/dashboard/'+encodeURIComponent(selectedGuildId)+'/'+section);render()}function render(){const path=location.pathname;if(path==='/'||path==='/login')document.getElementById('view').innerHTML=login();else if(path==='/servers'){document.getElementById('view').innerHTML=servers();loadGuilds()}else{const match=path.match(/^\\/dashboard\\/([^/]+)\\/([^/]+)$/);if(!match){location.href='/servers';return}selectedGuildId=decodeURIComponent(match[1]);current=names[match[2]]?match[2]:'overview';document.getElementById('view').innerHTML=dashboard();loadSummary();loadSettings()}}window.onpopstate=render;render();
</script></body></html>`;
...
// ============================================================
// ============================================================
// API بيانات السيرفر الحقيقي المختار
// كل طلب يتحقق من OAuth والصلاحية ووجود البوت داخل guildId.
// ============================================================
function authorizedBotGuild(req, res) {
  const session = requireSession(req, res);
  if (!session) return null;
  if (!managedGuilds(session).some((guild) => guild.id === req.params.guildId)) {
    res.status(403).json({ error: "forbidden_guild" });
    return null;
  }
  const guild = client.guilds.cache.get(req.params.guildId);
  if (!guild) {
    res.status(409).json({ error: "bot_not_installed", message: "البوت غير موجود داخل هذا السيرفر." });
    return null;
  }
  return guild;
}
app.get("/api/guilds/:guildId/summary", async (req, res) => {
  const guild = authorizedBotGuild(req, res);
  if (!guild) return;
  ...
  res.json({
    guild: { id: guild.id, name: guild.name, icon: guild.iconURL({ size: 128 }), memberCount: guild.memberCount ?? guild.members.cache.size, online, channels: guild.channels.cache.size, roles: Math.max(0, guild.roles.cache.size - 1) },
    bot: { ready: client.isReady(), user: client.user?.tag || null },
    permission: true,
    protection: config.antiNuke || config.antiLinks || config.antiSpam ? "مفعلة" : "متوقفة",
  });
});
...
app.get("/api/guilds/:guildId/members", async (req, res) => { const guild = authorizedBotGuild(req, res); if (!guild) return; await guild.members.fetch().catch(() => null); res.json({ members: [...guild.members.cache.values()].slice(0, 200).map((member) => ({ id: member.id, username: member.user.username, displayName: member.displayName, avatar: member.displayAvatarURL({ size: 64 }), roles: member.roles.cache.filter((role) => role.id !== guild.id).map((role) => role.id), bot: member.user.bot, joinedAt: member.joinedTimestamp })) }); });
app.get("/api/guilds/:guildId/channels", async (req, res) => { const guild = authorizedBotGuild(req, res); if (!guild) return; res.json({ channels: [...guild.channels.cache.values()].map((channel) => ({ id: channel.id, name: channel.name, type: channel.type, parentId: channel.parentId, position: channel.position })) }); });
app.get("/api/guilds/:guildId/roles", async (req, res) => { const guild = authorizedBotGuild(req, res); if (!guild) return; res.json({ roles: [...guild.roles.cache.values()].filter((role) => role.id !== guild.id).sort((a, b) => b.position - a.position).map((role) => ({ id: role.id, name: role.name, color: role.hexColor, position: role.position, managed: role.managed, mention: mentionRole(role) })) }); });
app.post("/api/settings/:guildId", (req, res) => { const guild = authorizedBotGuild(req, res); if (!guild) return;
...
// Exports are harmless for Render and useful for automated integration tests.
module.exports = { app, client, commands };
