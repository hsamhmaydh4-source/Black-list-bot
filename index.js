...
function servers(){return bg()+'<header class="picker-head">'+logo()+'<button class="secondary" style="width:auto;margin:0" onclick="location.href=&quot;/&quot;">العودة</button></header><main class="picker"><div class="eyebrow">▣ مساحة السيرفرات</div><h1>اختر السيرفر الذي<br><em>تريد إدارته.</em></h1><p class="muted">اختر السيرفر الذي تريد إدارته من المساحة الموحدة.</p><div class="demo">وضع العرض — القائمة قابلة للربط مع Discord API</div><div class="servers">${[
  ["NEXUS COMMUNITY", "NC", "24,892", ""],
  ["VOID GAMING", "VG", "8,421", "cyan"],
  ["CREATOR HUB", "CH", "3,108", "amber"],
]
  .map(
    (x) =>
      '<button class="server" onclick="location.href=&quot;/overview&quot;"><div class="avatar ' +
      x[3] +
      '">' +
      x[1] +
      '</div><div class="info"><strong>' +
      x[0] +
      "</strong><span>" +
      x[2] +
      ' عضو</span></div><div class="state"><span class="dot"></span>متصل</div><b>‹</b></button>',
  )
  .join(
    "",
  )}<button class="add"><div class="avatar">+</div><div class="info"><strong>إضافة سيرفر جديد</strong><small>ربط NEXUS بسيرفر آخر</small></div><b>←</b></button></div></main>'}
...
async function loadSettings(){try{const response=await fetch('/api/settings/'+dashboardGuildId);const data=await response.json();document.querySelectorAll('[data-setting]').forEach(el=>{el.classList.toggle('off',data[el.dataset.setting]===false)})}catch(e){console.warn('settings load failed',e)}}async function toggleSetting(el){el.classList.toggle('off');const payload={};document.querySelectorAll('[data-setting]').forEach(item=>payload[item.dataset.setting]=!item.classList.contains('off'));await fetch('/api/settings/'+dashboardGuildId,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).catch(()=>null)}async function saveSetting(){const payload={};document.querySelectorAll('[data-setting]').forEach(item=>payload[item.dataset.setting]=!item.classList.contains('off'));const response=await fetch('/api/settings/'+dashboardGuildId,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).catch(()=>null);if(response&&response.ok)alert('تم حفظ إعدادات البوت بنجاح.')}function go(section){current=section;history.pushState({},'', '/'+section);render()}function render(){const path=location.pathname;if(path==='/'||path==='/login')document.getElementById('view').innerHTML=login();else if(path==='/servers')document.getElementById('view').innerHTML=servers();else{current=path.slice(1);if(!names[current])current='overview';document.getElementById('view').innerHTML=dashboard();loadSettings()}}window.onpopstate=render;render();
</script></body></html>`;
...
app.get("/auth/discord", (_req, res) =>
  res.redirect(process.env.DISCORD_OAUTH_URL || "/servers"),
);
